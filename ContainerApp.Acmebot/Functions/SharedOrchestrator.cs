...
namespace ContainerApp.Acmebot.Functions
{
    public class SharedOrchestrator
    {
        ...
        public async Task IssueCertificate([OrchestrationTrigger] IDurableOrchestrationContext context)
        {
            ...
            certificatePolicy = await activity.MergeExistingCertificate(certificatePolicy);

            if(!certificatePolicy.ReuseKey && !certificatePolicy.Expiring)
            { 
                ...
                certificatePolicy = await activity.UploadCertificate((certificatePolicy, orderDetails, rsaParameters));
            ...
            await activity.SendCompletedEvent((certificatePolicy.FriendlyName, certificatePolicy.notAfter, certificatePolicy.DnsNames[0]));
        }
    }
}
