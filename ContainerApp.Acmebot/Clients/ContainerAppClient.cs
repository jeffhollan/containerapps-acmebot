...
namespace ContainerApp.Acmebot
{
    public class ContainerAppClient
    {
        ...
        public async Task UploadCertificateAsync(CertificatePolicyItem certificatePolicy, byte[] pfxBlob, string password)
        {
            var token = await _credential.GetTokenAsync(new TokenRequestContext(new[] { "https://management.azure.com/", "https://management.core.windows.net/" }));
            ...
            await httpClient.SendAsync(new HttpRequestMessage(HttpMethod.Post, $"/subscriptions/411a9cd0-f057-4ae5-8def-cc1ea96a3933/resourceGroups/d-certtest/providers/Microsoft.App/managedEnvironments/certtest/certificates/${certificatePolicy.CertificateName}?api-version=2022-01-01-preview")
            {
